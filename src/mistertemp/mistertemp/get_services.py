...
def get_services(url):
    ...
    for letter1 in list_alphabet:
        browser.find_element_by_class_name('react-autosuggest__input').send_keys(letter1)
        for letter2 in list_alphabet:
            browser.find_element_by_class_name('react-autosuggest__input').send_keys(letter2)
            try:
                for i in range(1,40):
                    list_jobs.append(browser.find_elements_by_xpath("//li[@data-suggestion-index='"+str(i)+"']")[0].text)
            except:
